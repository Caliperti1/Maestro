...
| 9.3b Semantic occurrence edit | Say: `Shift my L3 collaborative autonomy standup for only today from 11 to 1330 ET.` | Maestro resolves the event from rough title, L3 domain, date, time, and recurrence evidence; it moves only today's 30-minute occurrence to 1:30 PM and leaves the series unchanged. It asks only if ranked candidates remain genuinely ambiguous. | Knowledge action results, Calendar day/week view, recurrence exception metadata. | Automated; human run pending |
