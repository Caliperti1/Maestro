...
| 9.3a Recurring-event clarification | Say: `Create an USMA Collaborative Autonomy Standup every Monday through Thursday from now until the end of the year.` When Maestro asks for the missing time, reply only: `11-1130`. | Maestro treats the terse reply as the answer to its pending question, creates one event from 11:00 to 11:30 AM Eastern, and uses December 31 of the first occurrence's year as the recurrence end. The Calendar remains usable. | Chat history, event detail, and Calendar week view. | Not run |
| 9.3b Semantic occurrence edit | Say: `Shift my USMA collaborative autonomy standup for only today from 11 to 1330 ET.` | Maestro resolves the event from rough title, USMA domain, date, time, and recurrence evidence; it moves only today's 30-minute occurrence to 1:30 PM and leaves the series unchanged. It asks only if ranked candidates remain genuinely ambiguous. | Knowledge action results, Calendar day/week view, recurrence exception metadata. | Automated; human run pending |
| 9.3c Recurring todo | Say: `Build and send Perti Labs invoices every month by the fifth. It usually takes 90 minutes.` Then mark the current occurrence done. | Maestro creates one active monthly series with ordinary Todo occurrences. Completing this month's occurrence leaves the series active and the following month open. The Todo UI shows one grouped recurring card with pause/end controls. | Knowledge action results, Todo list, recurring series API, calendar projection when scheduled. | Automated; human run pending |
...
| 9.11 Attention boundary | Ingest a standalone todo or unresolved personal decision that is not blocking a workflow. | The item appears in Todos as open work and does not appear in Needs Attention. Needs Attention contains only resumable workflow blockers, RFIs, approval gates, or terminal failures requiring Chris. | Todos and Needs Attention panels. | Automated; human run pending |
